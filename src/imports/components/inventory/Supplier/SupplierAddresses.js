				...
				<TableRow key={listVariable.get('variableName')}>
					...
					<TableData width="8%" left="75%">
						<TableHeaderInner>
							<SelectWrapper>
							<Select
                            value={{ value: listVariable.get('values').get('country'), label: listVariable.get('values').get('country') }}
                            onChange={(option) => {
                                this.onChange({ target: { name: 'country', value: option.value } }, listVariable.get('variableName'))
                            }}
                            options={this.props.variables.Country !== undefined ?
                                this.props.variables.Country.map((variable) => { return { value: variable.variableName, label: variable.variableName } }) : []}
                        />
							</SelectWrapper>
