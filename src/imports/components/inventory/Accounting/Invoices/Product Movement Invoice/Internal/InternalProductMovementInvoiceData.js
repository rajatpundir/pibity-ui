...
class InternalProductMovementInvoiceData extends React.Component {
	constructor(props) {
		super();
		this.state = {
			open: false,
		...
		return (
			<React.Fragment key={this.props.data.variableName}>
				...
				<TableRow onClick={this.handleRowClick} key={this.props.data.variableName}>
					...
					<TableData width="10%">
						<TableHeaderInner>{this.props.data.values.productMovementOrder}</TableHeaderInner>
					</TableData>
					<TableData width="10%">
...
export default withStyles(styles)(InternalProductMovementInvoiceData);
